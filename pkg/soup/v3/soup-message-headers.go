...
// extern void _gotk4_soup3_MessageHeadersForEachFunc(char*, char*, gpointer);
import "C"
...
// MessageHeadersType: value passed to messageheaders.New to set certain default
// behaviors.
...
//
// Beware that unlike messageheaders.GetList, this processes the headers
...
// if you call messageheaders.Append multiple times with the same name, then the
// I/O code will output multiple copies of the header when sending the message
// to the remote implementation, which may be required for interoperability in
// some cases.)
//
...
func (hdrs *MessageHeaders) ForEach(fn MessageHeadersForEachFunc) {
	...
	_arg0 = (*C.SoupMessageHeaders)(gextras.StructNative(unsafe.Pointer(hdrs)))
	_arg1 = (*[0]byte)(C._gotk4_soup3_MessageHeadersForEachFunc)
...
// ContentDisposition looks up the "Content-Disposition" header in hdrs,
// parses it, and returns its value in *disposition and *params.
//
...
// returns its value in *content_type and *params.
//
...
//
// Use this for headers whose values are comma-delimited lists, and which
// are therefore allowed to appear multiple times in the headers. For
// non-list-valued headers, use messageheaders.GetOne.
//
// If name appears multiple times in hdrs, messageheaders.GetList will
...
//
// Server has built-in handling for range requests. If your server handler
// returns a SOUP_STATUS_OK response containing the complete response body
// (rather than pausing the message and returning some of the response body
...
//
// See messageheaders.GetContentDisposition for a discussion of how
...
// messageheaders.SetContentLength allows you to put the correct content length
// into the response without needing to waste memory by filling in a response
// body which won't actually be sent.
//
// The function takes the following parameters:
//
...
// values.
//
// (Note that total_length is the total length of the entire resource that this
// is a range of, not simply end - start + 1.)
//
// server has built-in handling for range requests, and you do not normally need
// to call this function youself. See messageheaders.GetRanges for more details.
//
// The function takes the following parameters:
//
//   - start of the range.
